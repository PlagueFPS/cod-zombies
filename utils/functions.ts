...
export const calculateTimeToRead = (contentPath: string) =>
  Effect.gen(function* () {
    const wordCount = yield* Effect.tryPromise({
      try: () =>
        readFile(path.join(process.cwd(), contentPath), { encoding: "utf-8" }),
      catch: (error) =>
        new ReadFileError({ message: "Failed to read file", cause: error }),
    }).pipe(
  ...
  Effect.gen(function* () {
    const buffer = yield* Effect.try({
      try: () => Buffer.from(token, "base64url").toString(),
        ...
        new TokenVerificationError({ message: "Invalid Token", cause: error }),
    });
    ...
    if (Duration.greaterThan(now, expiresIn.value)) {
      return yield* new TokenExpirationError({
        ...
        cause: new Error(
          `Token expired at ${new Date(expiresIn.value).toISOString()}`,
        ),
      });
    }
