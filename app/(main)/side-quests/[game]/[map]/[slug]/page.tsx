...
import { ChangedBadge, ComingSoonBadge, DraftBadge, NewBadge } from "@/components/custom-badges/custom-badges"
...
import { getQuestBySlug, getQuests, type MinifiedSideQuest, type SideQuest } from "@/data/side-quests"
import { env } from "@/env"
							...
							<div className="flex items-center justify-between text-muted-foreground text-sm">
								<div className="flex flex-col-reverse items-start justify-center gap-2 pb-6 xl:flex-row xl:pb-0">
									<div className="flex items-center gap-1">
										...
										<span>Updated: {new Date(q.updatedAt).toLocaleDateString(undefined, DATE_OPTIONS)}</span>
									</div>
...
const PrevOrNextQuestCard = ({
	...
	return (
		<CustomLink
			href={href}
			className={cn(
				"group w-full max-w-sm overflow-hidden rounded-lg border-2 shadow-sm transition-transform hover:scale-105 hover:border-primary lg:max-w-xl dark:shadow-none",
				{
		...
		>
			...
			>
				...
				>
					<FeaturedImage featuredImage={quest.image} sizes="32px" quality={1} className="scale-110" />
				</div>
				...
				<div className="relative z-20 flex h-full w-full flex-col justify-center gap-2 px-4 pt-4 xl:pt-6">
					<h2
						className={cn(
							"dark:dark-text-gradient bg-linear-to-b bg-clip-text font-extrabold text-gradient text-transparent text-xl",
