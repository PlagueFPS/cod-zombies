...
export default async function ZombieImage({ params }: PageProps<"/bestiary/[slug]">) {
	...
	if (!zombie?.image.url) {
		console.error("No image URL found for zombie", zombie?.slug)
		return null
	}

	const serverUrl = getServerUrl()
	const imageRes = await fetch(`${serverUrl}${zombie.image.url}`)
	if (!imageRes.ok) {
		console.error("Failed to fetch image for zombie", zombie.slug)
		return null
	}
