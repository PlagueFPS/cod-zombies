...
const NotFoundButtons = <T extends string>({
	...
	return (
		<div className="flex w-fit flex-col items-center justify-between gap-8 sm:flex-row">
			{newItems.length > 0 ? (
				newItems.map(item => (
					<Button nativeButton={false} key={`${item.title}-${item.href}`} variant={"outline"} render={<CustomLink href={item.href}>View {item.title}</CustomLink>} />
				))
			) : (
				<>
					<Button nativeButton={false} variant={"outline"} render={<CustomLink href="/main-quests">View all Main Quests</CustomLink>} />
					<Button nativeButton={false} variant={"outline"} render={<CustomLink href="/side-quests">View all Side Quests</CustomLink>} />
					<Button nativeButton={false} variant={"outline"} render={<CustomLink href="/bestiary">View all Zombies</CustomLink>} />
					<Button nativeButton={false} variant={"outline"} render={<CustomLink href="/relics">View all Cursed Relics</CustomLink>} />
					<Button nativeButton={false} variant={"outline"} render={<CustomLink href="/maps">View all Interactive Maps</CustomLink>} />
				</>
			)}
		</div>
	)
