...
import { CRS, LatLng, LatLngBounds, type LatLngTuple, type Map as LeafletMap, type LeafletMouseEvent } from "leaflet"
...
export default function InteractiveMap({ mapConfig }: IInteractiveMap) {
	...
	const convertToLeafletCoords = useCallback(
		({ x, y }: Location): LatLng => {
			...
			return new LatLng(imageDimensions.height - y * imageDimensions.height, x * imageDimensions.width)
		},
		[imageDimensions],
	)
	...
	return (
		<MapContainer
			key={mapConfig.id}
			ref={mapRef}
			center={imageDimensions ? [imageDimensions.height / 2, imageDimensions.width / 2] : [1024, 1024]}
			zoom={0}
			minZoom={-2}
			maxZoom={3}
			crs={CRS.Simple}
		...
		>
			...
			{imageDimensions && <ImageOverlay key={mapConfig.id} url={mapConfig.image} bounds={getImageBounds()} />}
