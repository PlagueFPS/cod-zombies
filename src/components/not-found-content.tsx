...
import { NotFoundBreadcrumbs } from "@/components/not-found-breadcrumbs"
			...
			{newItems.length > 0 ? (
				newItems.map(item => (
					<Button
						...
						variant={"outline"}
						render={
							<CustomLink to={item.href} params={item.params} search={item.search}>
								View {item.title}
							</CustomLink>
						}
					/>
				...
				<Button
					nativeButton={false}
					variant={"outline"}
					render={<CustomLink to="/">Back to Home</CustomLink>}
				/>
			)}
		</div>
	)
