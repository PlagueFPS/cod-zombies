...
const program = Effect.gen(function* () {
	...
	yield* Effect.forEach(media, file =>
		Effect.gen(function* () {
			if (!file.startsWith("the-giant-")) return
			...
			yield* fs.writeFile(
				path.join(process.cwd(), "./content/images/the-giant", file.replace(extension, ".webp")),
				image,
			)
			yield* Effect.log(`Optimized: ${shouldOptimize}; moved ${file} to the-giant`)
