...
const program = Effect.gen(function* () {
	...
	yield* Effect.forEach(media, file =>
		Effect.gen(function* () {
			...
			yield* fs.writeFile(
				path.join(process.cwd(), "./content/images/ix", file.replace(extension, ".webp")),
				image,
			)
			yield* Effect.log(`Optimized: ${shouldOptimize}; moved ${file} to ix`)
