...
export const categoryHandlers = {
	general: (m: EncodedMapMarker) => (m.type === "label" ? m.type : m.id),
	equipment: (m: EncodedMapMarker) => (m.type === "weapon-wall-buy" ? m.type : m.id),
	upgrades: (m: EncodedMapMarker) => (m.type === "perk" ? m.type : m.id),
	objectives: (m: EncodedMapMarker) => m.id,
	transportation: (m: EncodedMapMarker) => m.id,
	intel: (m: EncodedMapMarker) => m.id,
} as const
